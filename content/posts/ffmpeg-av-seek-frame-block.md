...
- 正常的 HLS 录制：一次直播会话对应一组固定编码参数（分辨率、帧率、SPS/PPS 等），录制出来的 ts 文件编码格式从头到尾一致。
- 出问题的录制：直播过程中源流切换过编码（比如转码、推流端重连换了参数、分辨率自适应等），**但服务端没有在编码切换时重置录制上下文**，直接把不同编码格式的数据拼到了**同一个 ts 文件**里。

这就导致一个 ts 文件内部存在编码格式不一致的情况——比如前半段是 H.264 baseline，后半段突然变成 main profile；或者参数（分辨率、SPS/PPS）发生了变化。

`av_seek_frame` 在 seek 时，需要定位到目标位置并读取附近的数据来重建解码上下文。当它 seek 到编码切换的边界附近时，demuxer/解析逻辑会陷入混乱：它读到的数据和它根据索引期望的数据对不上，于是不停往后读、试图找到一个「合理」的位置，结果陷入死循环般地一直读不到头——**最终表现为永久阻塞**。
...
理想情况下，**根治的办法当然是修服务端录制逻辑**：在检测到编码参数变化时，重置录制上下文，要么切到新的 ts 文件，要么至少保证单个 ts 内编码一致。
