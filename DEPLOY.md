...
### Q: Actions 显示成功 ✅，但新文章没出现在网站上？

很可能是**文章日期在未来**。Hugo 默认 `buildFuture = false`，会**静默跳过**日期晚于构建时间的文章，且**不报错**——所以 Actions 显示绿色 ✅、Pages 数量正常，但文章就是不出现。

**排查方法**：对比文章 front matter 里的 `date` 和 GitHub Actions 的构建时间（在 Actions 日志里能看到 UTC 时间，记得换算成 +08:00）。如果文章 date 比构建时间晚几秒到几分钟，就是这个原因。

**修复方法**：把文章 `date` 往前调一点（早于构建时间即可），重新 push。或者如果确实需要定时发布，在 `hugo.toml` 加：

```toml
buildFuture = true
```

> 💡 建议：写文章时 `date` 直接用**当前或更早的时间**，避免踩这个隐蔽的坑。
